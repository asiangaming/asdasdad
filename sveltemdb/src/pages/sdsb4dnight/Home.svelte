<script>
  ...
  const firebaseConfig = {
    ...
    databaseURL:
      "https://dazzling-pillar-328210-default-rtdb.asia-southeast1.firebasedatabase.app",
  ...
  const EditData = (e, tanggal, prize_1, prize_2, prize_3) => {
    ...
    tanggal_keluaran = dayjs(tanggal).format("YYYY-MM-DD");
    myModal_newentry = new bootstrap.Modal(
      ...
      switch (tipe) {
        case "prize1":
          ...
          set(ref(db, "sdsb4dnight"), {
            ...
            nextdraw: dayjs(tanggal_keluaran)
              .add(1, "day")
              .format("YYYY-MM-DD"),
            prize1: prize,
            prize2: prize2,
  ...
  function generate(field) {
    let numbergenerate = (Math.floor(Math.random() * 10000) + 10000)
      .toString()
      .substring(1);
    switch (field) {
      case "prize1":
                ...
                {#each listHome as rec}
                  <tr>
                      ...
                      <i
                        on:click={() => {
                          EditData(
                            rec.sdsbnight_id,
                            rec.sdsbnight_date,
                            rec.sdsbnight_prize1,
                            rec.sdsbnight_prize2,
                            rec.sdsbnight_prize3
                          );
                        }}
                        class="bi bi-pencil"
                      />
                    ...
                    <td
                      NOWRAP
                      style="text-align: center;vertical-align: top;font-size: {table_body_font};"
                      >{dayjs(rec.sdsbnight_date).format("YYYY-MM-DD")}</td
                    >
                    <td
                      NOWRAP
                      style="text-align: center;vertical-align: top;font-size: {table_body_font};"
                    ...
                    <td
                      ...
                      >{rec.sdsbnight_create}</td
                    >
                    <td
                      NOWRAP
                      style="text-align: left;vertical-align: top;font-size: {table_body_font};"
                      >{rec.sdsbnight_update}</td
                    >
                  </tr>
                {/each}
              </tbody>
